...
switch(state)
	...
	case "moving": //Moving 
		if instance_exists(target) and can_move = true {
			...
			if distance_to_object(target) <= e_target_range & collision_line(x,y,target.x,target.y,obj_block,true,true) 
				{ //If enemy is within range and has line of sight, use mp_step
					move_state = 0 //Debug only
					path_end();
					mp_potential_step_object(target.x,target.y,e_speed,obj_block)
				...
				if alarm[0] == -1 {
						...
						alarm[0] = e_opt_time; //Reset the scan timer
