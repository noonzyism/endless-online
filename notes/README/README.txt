...
client changes movement/direction:
	-client sends position, current speeds, image angle, and shooting state to server (STATE)
	-client sends image angle and shooting state (true/false) 
	-client calcs new position locally
	-client calcs if shot hit anybody locally and alters peer HPs appropriately

server receives STATE packet:
	-server verifies, updates peer instance position from given speeds, shooting state, and angle
