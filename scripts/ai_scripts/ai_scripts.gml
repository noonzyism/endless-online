...
{
	if distance_to_object(argument1) < argument0 or distance_to_object(ctrl_match_survival.blockade_object) < argument0 { 
...
function switch_target(argument0, argument1) 
{
	...
	var bar = instance_nearest(x,y,ctrl_match_survival.blockade_object); 
	var list = ds_priority_create(); //Create a priority list
	if distance_to_object(bar) < argument1 {
		tar = bar;		
	}else {
		for (var i = 0; i < array_length(ctrl_match_survival.objective); i++) { //Loops through all objectives
			ds_priority_add(list,ctrl_match_survival.objective[i].id,distance_to_object(ctrl_match_survival.objective[i])) //Adds the objects and distances | Compares them
		}
