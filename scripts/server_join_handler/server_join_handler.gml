function server_join_handler() {
	...
	ctrl_server.clients[i, 0] = instance_create_depth(100, 100, 100, obj_peer);
	ctrl_server.clients[i, 1] = _username;
	ctrl_server.clients[i, 2] = _socket;

	show_debug_message("Created client [" + string(ctrl_server.clients[i, 0]) + ", " + string(ctrl_server.clients[i, 1]) + ", " + string(ctrl_server.clients[i, 2]) + "]");

	var client_id = i;

	show_debug_message("Server received join request: [" + string(ctrl_server.clients[client_id, 1]) + ", " + string(client_id) + "]");

	//set and send out match info for this client to all clients
	client_instance = ctrl_server.clients[i, 0];
	...
	broadcast_sync(_session, client_id, 0, true);
}
