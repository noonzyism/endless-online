...
function client_ack_handler() {
	...
	&& (_sendtick == ctrl_client.tick_lastsent || _sendtick == 0)) {
		...
		if (_sendtick != 0 && _xspeed == obj_player.xspeed && _yspeed == obj_player.yspeed) {
			obj_player.x = dest[0];
			obj_player.y = dest[1];
		}
			
		if (actual_x != dest[0] || actual_y != dest[1]) {
			if (global.debug) ui_notify(0, "Host sync: [" + string(_x) + ", " + string(_y) + "] Calc'd: [" + string(dest[0]) + ", " + string(dest[1]) + "] Actual: [" + string(actual_x) + ", " + string(actual_y) + "]");
