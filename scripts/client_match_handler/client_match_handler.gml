...
function client_match_handler() {
	...
	if (_id == ctrl_client.clientId) {
		...
		if (ctrl_client.peers[_id] == -1) {
			ctrl_client.peers[_id] = instance_create_depth(32, 32, 100, obj_peer);
		}
	
		var peer = ctrl_client.peers[_id]; //get this peer's local instance id
